...
class StoreRegToGlbIterateByRow(AbstractInstruction):
  def __init__(self,
               vm: VM,
               dest: Symbol,
               src: Symbol,
               ...
               beta: float,
               num_threads: int):
    super(StoreRegToGlbIterateByRow, self).__init__(vm)
  ...
  def gen_code(self, writer):
    ...
    with writer.If(self.gen_mask_threads(self._num_threads)):
      writer.Pragma("unroll")
      with writer.For(f'int m = 0; m < {dest_matrix.get_actual_num_rows()}; ++m'):
        rhs = "{}[{} * {} + m]".format(dest_name,
                                       self._vm.get_lexic().thread_idx_x,
                                       dest_matrix.num_rows)

        real_suffix = 'f' if precision == "float" else ''

        src_access = '' if self._src.obj.size == 1 else '[m]'
        if not isinstance(self._alpha, float):
          lhs = f'{self._alpha} * {self._src.name}{src_access}'
        else:
          if self._alpha == 1.0:
