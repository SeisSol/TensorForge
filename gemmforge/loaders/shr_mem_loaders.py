    ...
    def generate_scr(self, file, in_symbol):
        ...
        if num_hops > 0:
            if num_hops > self.manual_unroll_threshold:
                ...
                with file.For("int i = 0; i < {}; ++i".format(num_hops)):
                    ...
                    shr_mem_index = "{} + i * {}".format(self._lexic.thread_idx_x, self.num_active_threads)
                ...
                for counter in range(num_hops):
                    shr_mem_index = "{} + {}".format(self._lexic.thread_idx_x, counter * self.num_active_threads)
        ...
        if (self.shm_volume % self.num_active_threads) != 0:
            ...
            with file.If("{} < {}".format(self._lexic.thread_idx_x, residue)):
                shr_mem_index = "{} + {}".format(self._lexic.thread_idx_x, num_hops * self.num_active_threads)
        ...
        with file.For("int i = 0; i < {}; ++i".format(self.matrix.get_actual_num_cols())):
            ...
            if num_hops > 0:
                if num_hops > self.manual_unroll_threshold:
                    ...
                    with file.For("int counter = 0; counter < {}; ++counter".format(num_hops)):
                        shr_mem_index = "{} + counter * {} + i * {}".format(self._lexic.thread_idx_x,
                                                                            self.num_active_threads,
                                                                            self.lid_dim)

                        glob_mem_index = "{} + counter * {} + i * {}".format(self._lexic.thread_idx_x,
                                                                             self.num_active_threads,
                    ...
                    for counter in range(num_hops):
                        ...
                        shr_mem_index = "{} + {} + i * {}".format(self._lexic.thread_idx_x, offset, self.lid_dim)

                        glob_mem_index = "{} + {} + i * {}".format(self._lexic.thread_idx_x, offset, self.matrix.num_rows)
                        file.Assignment("{}[{}]".format(self.out_symbol, shr_mem_index),
                                        "{}[{}]".format(in_symbol, glob_mem_index))
            ...
            if (self.lid_dim % self.num_active_threads) != 0:
                ...
                with file.If("{} < {}".format(self._lexic.thread_idx_x, residue)):
                    ...
                    shr_mem_index = "{} + {} + i * {}".format(self._lexic.thread_idx_x, finial_offset, self.lid_dim)
                    glob_mem_index = "{} + {} + i * {}".format(self._lexic.thread_idx_x, finial_offset, self.matrix.num_rows)
                    file.Assignment("{}[{}]".format(self.out_symbol, shr_mem_index),
                                    "{}[{}]".format(in_symbol, glob_mem_index))
