...
def build_pointwise(operation, *args, **kwargs):
  pointwise = Pointwise(*args, **kwargs)
  pointwise.operation = operation
  return pointwise

class Pointwise(AbstractInstruction, Gemm):
  def __init__(self,
               context: Context,
               trans_a: bool,
               trans_b: bool,
  ...
  def gen_code(self, writer: Writer):
    ...
    with writer.If(self.gen_mask_threads(self._op1_view.get_dim_size(0))):
      ...
      with writer.For(f'int k = 0; k < {k_range}; ++k'):
        ...
        writer(f'{self._fp_as_str} value1 = {self._op1.name}[{address}];')
        writer(f'{self._fp_as_str} value2 = {self._op2.name}[{address}];')
  ...
  def _write_operation(self):
    ...
    return self._context.get_vm().get_lexic().get_operation(self.operation, fptype, 'value1', 'value2')
