...
class MultilinearMultiInstruction(ComputeInstruction):
    def __init__(self,
               context: Context,
               dest: List[Symbol],
               ops: List[SymbolView],
               target: List[List[Union[None, List[int]]]],
               prev: List[Union[None, Symbol]],
               productOperation: ReductionOperator,
               sumOperation: ReductionOperator,
               prefer_align: bool,
               num_threads: int,
               blockcount: int=1):
        super(MultilinearMultiInstruction, self).__init__(context)
        self._dest = dest
        self._ops = ops
        self._target = target
    ...
    def _analyze(self):
        ...
        for i, op in enumerate(self._ops):
            for k in range(len(self._dest)):
                if self._target[k][i] is not None:
                    for j in range(op.bbox.rank()):
                        targetrank = max(self._target[k][i][j] + 1, targetrank)
        ...
        for i, op in enumerate(self._ops):
            ...
            for k in range(len(self._dest)):
                if self._target[k][i] is not None:
                    for j in range(op.bbox.rank()):
                        if self._target[k][i][j] < 0:
                            if self._target[k][i][j] not in preKs:
                                preKs[self._target[k][i][j]] = (op.bbox.lower()[j], op.bbox.upper()[j])
                            preKs[self._target[k][i][j]] = (max(preKs[self._target[k][i][j]][0], op.bbox.lower()[j]), min(preKs[self._target[k][i][j]][1], op.bbox.upper()[j]))

                            prev = opdim[j]
                            opdim[j] = f'k{-self._target[k][i][j] - 1}'

                            assert prev in ['', opdim[j]]
                        else:
                            self._ns[self._target[k][i][j]] = (max(self._ns[self._target[k][i][j]][0], op.bbox.lower()[j]), min(self._ns[self._target[k][i][j]][1], op.bbox.upper()[j]))

                            prev = opdim[j]
                            opdim[j] = f'n{self._target[k][i][j]}'
        ...
        for dest in self._dest:
            ...
            dest.data_view._bbox._lower = [l for l,_ in self._ns]
            dest.data_view._bbox._upper = [u for _,u in self._ns]
    ...
    def gen_code_inner(self, writer: Writer):
        ...
        if any(prev is not None for prev in self._prev):
...
#                if op.symbol.
                    ...
                    for i, t in enumerate(tgt):
                        if t is not None:
                            if outstr is None:
                                ...
                                outstr = self._productOperation.format(outstr, f"data{i}")

                    if outstr is not None:
                        dest.load(writer, self._context, 'value', [varlist[loopmap[f'n{i}']] for i,_ in enumerate(self._ns)], False)
                        writer(f'{self._fp_as_str} newvalue = {self._sumOperation.format("value", outstr)};')
                        dest.store(writer, self._context, 'newvalue', [varlist[loopmap[f'n{i}']] for i,_ in enumerate(self._ns)], False)
    ...
    def _add_to_prev(self, writer: Writer):
        ...
        def nonlead_writer(varlist):
            for prev, dest in zip(self._prev, self._dest):
                if prev is not None:
                    dest.load(writer, self._context, 'value', [varlist[loopmap[f'n{i}']] for i,_ in enumerate(self._ns)], False)
                    prev.load(writer, self._context, 'oldvalue', [varlist[loopmap[f'n{i}']] for i,_ in enumerate(self._ns)], False)
