...
class BatchedOperationsAux:
  ...
  def _get_ptr_type(self, addressing):
    return '**' if addressing == tensorforge.common.basic_types.Addressing.PTR_BASED else '*'

  def deduce_addresing(self, term):
    if term.is_compute_constant:
      return tensorforge.common.basic_types.Addressing.NONE
    if term.is_temporary:
      return tensorforge.common.basic_types.Addressing.STRIDED
    else:
      return tensorforge.common.basic_types.Addressing.PTR_BASED
