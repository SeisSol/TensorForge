...
int main(int argc, char **argv) {
  ...
  {
    real *C__;
    _error = posix_memalign(reinterpret_cast<void **>(&C__), ALIGNMENT,
                            tensor::C::size() * sizeof(real));
    real *A__;
    _error = posix_memalign(reinterpret_cast<void **>(&A__), ALIGNMENT,
                            tensor::A::size() * sizeof(real));
    real *B__;
    _error = posix_memalign(reinterpret_cast<void **>(&B__), ALIGNMENT,
                            tensor::B::size() * sizeof(real));
    ...
    _time = _sw.stop();
    _nzflops = static_cast<double>(kernel::matmulAB::NonZeroFlops) * _reps /
               _time / 1.0e9;
    _flops = static_cast<double>(kernel::matmulAB::HardwareFlops) * _reps /
             _time / 1.0e9;
    printf("matmulAB,%u,%lf,%lu,%lu,%lf,%lf\n", _reps, _time,
           kernel::matmulAB::NonZeroFlops, kernel::matmulAB::HardwareFlops,
           _nzflops, _flops);
    free(C__);
    free(A__);
    free(B__);
  }
  {
    ...
    _time = _sw.stop();
    _nzflops = static_cast<double>(kernel::matmulATB::NonZeroFlops) * _reps /
               _time / 1.0e9;
    _flops = static_cast<double>(kernel::matmulATB::HardwareFlops) * _reps /
             _time / 1.0e9;
    printf("matmulATB,%u,%lf,%lu,%lu,%lf,%lf\n", _reps, _time,
           kernel::matmulATB::NonZeroFlops, kernel::matmulATB::HardwareFlops,
           _nzflops, _flops);
    free(C__);
    free(A__);
    free(B__);
  }
  {
    ...
    _time = _sw.stop();
    _nzflops = static_cast<double>(kernel::matmulABT::NonZeroFlops) * _reps /
               _time / 1.0e9;
    _flops = static_cast<double>(kernel::matmulABT::HardwareFlops) * _reps /
             _time / 1.0e9;
    printf("matmulABT,%u,%lf,%lu,%lu,%lf,%lf\n", _reps, _time,
           kernel::matmulABT::NonZeroFlops, kernel::matmulABT::HardwareFlops,
           _nzflops, _flops);
    free(C__);
    free(A__);
    free(B__);
  }
  {
    ...
    _time = _sw.stop();
    _nzflops = static_cast<double>(kernel::matmulATBT::NonZeroFlops) * _reps /
               _time / 1.0e9;
    _flops = static_cast<double>(kernel::matmulATBT::HardwareFlops) * _reps /
             _time / 1.0e9;
    printf("matmulATBT,%u,%lf,%lu,%lu,%lf,%lf\n", _reps, _time,
           kernel::matmulATBT::NonZeroFlops, kernel::matmulATBT::HardwareFlops,
           _nzflops, _flops);
    free(C__);
    free(B__);
    free(A__);
