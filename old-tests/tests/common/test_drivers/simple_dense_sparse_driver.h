...
/*
  The sole change of this driver compared to the simple driver is that it
  compares Dense x Sparse Matrix multiplication with Dense x Dense already
  implemented. The correctnesss of this test case relies and assumes that Dense
  x Dense is already correct. Furthermore, it does the test by multiplying both
  the Dense and Sparse version of B.
*/
...
public:
  ...
  TestDriver(int rowA, int colA, int rowB, int colB, int rowC, int colC,
             int NumElements, std::string matrix_b_type, bool transB)
      : m_rowA(rowA), m_colA(colA), m_rowB(rowB), m_colB(colB), m_rowC(rowC),
        m_colC(colC), m_SizeMatA{rowA * colA}, m_SizeMatB{rowB * colB},
        m_NumElements(NumElements), m_matrix_b_type(matrix_b_type),
        m_transB(transB), m_IsReady(true) {}
  ~TestDriver() {}

  void setParams(int rowA, int colA, int rowB, int colB, int rowC, int colC,
                 int NumElements, std::string matrix_b_type, bool transB);
  ...
  std::tuple<real *, real *, real *, real *, real *> getDeviceRawData() {
    return std::make_tuple(m_DeviceMatA, m_DeviceMatB_dense,
                           m_DeviceMatB_sparse, m_DeviceMatC1, m_DeviceMatC2);
  }

  std::tuple<real *, real *, real *, real *> getHostRawData() {
    return std::make_tuple(m_HostMatA, m_HostMatB_dense, m_HostMatB_sparse,
                           m_HostMatC);
  }

  std::tuple<real *, real *, real *> getRawResults();

  void retrieveResults(int NumRows, int LeadDim, int NumColumns, int Offset,
                       int Stride, int NumElements, bool sparseResult);

  bool checkEq(real Eps = 1e-5);

  double getDeviceAllocatedMemSize() {
    long long Size = (m_SizeMatA + m_SizeMatB * 2 + m_SizeMatC) *
...
protected:
  ...
  void initSparseMatrix(real *DenseVersionOfSparseMatrix, real *SparseMatrix,
                        int rowB, int colB, std::string matrix_b_type);
    ...
    if (matrix_b_type == "random") {
      ...
      if (colB % 2 == 0) {
        ...
        if (rowB % 2 == 0) {
          ...
          // Implementation specific of the element orders, in my case first row
          // has +1 element, when unequal then we get 1 too many
