...
public:
  ...
  TestDriver(int SizeMatA, int SizeMatB, int SizeMatC, int NumElements)
      : m_SizeMatA(SizeMatA), m_SizeMatB{SizeMatB}, m_SizeMatC(SizeMatC),
        m_NumElements(NumElements), m_IsReady(true) {}
  ~TestDriver() {}
  ...
  getShuffledDeviceData();
  std::tuple<real *, real *> getRawResults();

  void packResults(int NumRows, int LeadDim, int NumColumns, int Offset,
                   int Stride, int NumElements);
  std::tuple<PackedData, PackedData> getPackedResults() {
    return std::make_tuple(m_PackedHostResults, m_PackedDeviceResults);
  }

  template <class ComparatorType> bool isTestPassed(real Eps = 1e-5) {
    assert(!m_PackedHostResults.empty() != 0 &&
           "Host results has not been packed");
    assert(!m_PackedDeviceResults.empty() &&
           "Device results has not been packed");
  ...
  double getDeviceAllocatedMemSize() {
    long long Size =
        (m_SizeMatA + m_SizeMatB + m_SizeMatC) * m_NumElements * sizeof(real);
