...
void TestDriver::setParams(int SizeMatA, int SizeMatB, int SizeMatC,
                           int NumElements) {
...
void TestDriver::SetUp() {
  if (m_IsReady && !m_IsSet) {
    ...
    m_DeviceMatA = (real *)getDeviceAPI()->allocGlobMem(
        m_SizeMatA * m_NumElements * sizeof(real));
    CHECK_ERR;
    m_DeviceMatB = (real *)getDeviceAPI()->allocGlobMem(
        m_SizeMatB * m_NumElements * sizeof(real));
    CHECK_ERR;
    m_DeviceMatC = (real *)getDeviceAPI()->allocGlobMem(
...
void TestDriver::prepareData() {
  if (m_IsSet) {
    ...
    getDeviceAPI()->copyTo(m_DeviceMatA, m_HostMatA,
                           m_SizeMatA * m_NumElements * sizeof(real));
    CHECK_ERR;
    getDeviceAPI()->copyTo(m_DeviceMatB, m_HostMatB,
                           m_SizeMatB * m_NumElements * sizeof(real));
    CHECK_ERR;
    getDeviceAPI()->copyTo(m_DeviceMatC, m_HostMatC,
                           m_SizeMatC * m_NumElements * sizeof(real));
    CHECK_ERR;
  } else {
...
void *TestDriver::getTestStream() { return getDeviceAPI()->getDefaultStream(); }
...
TestDriver::getShuffledDeviceData() {
  if (m_IsSet) {
    return std::make_tuple(
        shuffleMatrices(m_DeviceMatA, m_SizeMatA, m_NumElements),
...
std::tuple<real *, real *> TestDriver::getRawResults() {
  if (m_IsSet) {
    getDeviceAPI()->copyFrom(m_ResultsFromDevice, m_DeviceMatC,
                             m_SizeMatC * m_NumElements * sizeof(real));
    CHECK_ERR;
...
void TestDriver::packResults(int NumRows, int LeadDim, int NumColumns,
    ...
    for (int Element = 0; Element < NumElements; ++Element) {
      ...
      for (int Col = 0; Col < NumColumns; ++Col) {
        for (int Row = 0; Row < NumRows; ++Row) {
          ...
          ElementFromHost[Index] =
              HostRawData[Row + Col * LeadDim + Stride * Element];
        }
      }
      ...
      for (int Col = 0; Col < NumColumns; ++Col) {
        for (int Row = 0; Row < NumRows; ++Row) {
          const int Index = Row + Col * NumRows;
          ElementFromDevice[Index] =
              DeviceRawData[Row + Col * LeadDim + Stride * Element];
        }
      }
