...
bool compare(real *host, const real *device, unsigned size, size_t numElements,
  ...
  for (int element = 0; element < numElements; ++element) {
    for (int index = 0; index < size; ++index) {
      real difference = std::abs(host[index + element * size] -
                                 device[index + element * size]);
      if (difference > eps) {

        std::cout << "at index " << index << ") " << "(bad) " << difference
                  << " = " << host[index + element * size] << " - "
                  << device[index + element * size] << std::endl;
