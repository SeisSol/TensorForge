...
#endif
inline constexpr tensorforge::LaunchInfo launch_info_kernel_5031e7df1dbaf2a0 = {{1, 16, 1}, 16, 16, 1, 16, 4096, false, true, 1};
tensorforge::LaunchConfig launch_config_kernel_5031e7df1dbaf2a0(size_t numElements0, void* streamPtr = nullptr);
void launcher_kernel_5031e7df1dbaf2a0(const float * m0, size_t m0_extraOffset, float * m1, size_t m1_extraOffset, size_t numElements0, unsigned * flags0 = nullptr, void* streamPtr = nullptr);
...
#endif
tensorforge::LaunchConfig launch_config_kernel_5031e7df1dbaf2a0(size_t numElements0, void* streamPtr) {
  (void)numElements0;
  (void)streamPtr;
...
void launcher_kernel_5031e7df1dbaf2a0(const float * m0, size_t m0_extraOffset, float * m1, size_t m1_extraOffset, size_t numElements0, unsigned * flags0, void* streamPtr) {
  const tensorforge::LaunchConfig config = launch_config_kernel_5031e7df1dbaf2a0(numElements0, streamPtr);
  ...
  kernel_kernel_5031e7df1dbaf2a0(stream, grid, block, m0, m0_extraOffset, m1, m1_extraOffset, numElements0, flags0);
  CHECK_ERR;
}


// === kernel ===
inline void kernel_kernel_5031e7df1dbaf2a0(sycl::queue *stream, sycl::range<3> group_count, sycl::range<3> group_size, const float * m0, size_t m0_extraOffset, float * m1, size_t m1_extraOffset, size_t numElements0, unsigned * flags0) {
        ...
        // tensorforge-meta: {"fp":"float","launch":{"active_threads":16,"block":[1,16,1],"cooperative":false,"lead_width":1,"mults_per_block":16,"persistent":true,"sections":[{"barrier":false,"mults_per_block":16,"shared_elements":1024}],"shared_bytes":4096,"shared_elements":1024,"threads_per_mult":16},"loops":[],"operands":[{"addressing":"strided","alias":"A","bbox":[[0],[40]],"name":"m0","ordered":false,"parts":1,"shape":[40],"variant":false},{"addressing":"strided","alias":"C","bbox":[[0],[40]],"name":"m1","ordered":false,"parts":1,"shape":[40],"variant":false}],"operations":[{"add":false,"dest":{"addressing":"pointer_based","bbox":[[0],[40]],"is_tmp":true,"name":"t0","offset":[0],"shape":[40]},"kind":"elementwise","op":"ABS","ops":[{"addressing":"strided","bbox":[[0],[40]],"is_tmp":false,"name":"m0","offset":[0],"shape":[40]}],"permute":[[0]],"scalars":[],"target":[[0]]},{"add":false,"dest":{"addressing":"pointer_based","bbox":[[],[]],"is_tmp":true,"name":"t1","offset":[],"shape":[]},"kind":"reduction","op":"max","ops":[{"addressing":"pointer_based","bbox":[[0],[40]],"is_tmp":true,"name":"t0","offset":[0],"shape":[40]}],"permute":[[0]],"target":[[-1]]},{"add":false,"dest":{"addressing":"strided","bbox":[[0],[40]],"is_tmp":false,"name":"m1","offset":[0],"shape":[40]},"kind":"elementwise","op":"MUL","ops":[{"addressing":"strided","bbox":[[0],[40]],"is_tmp":false,"name":"m0","offset":[0],"shape":[40]},{"addressing":"pointer_based","bbox":[[],[]],"is_tmp":true,"name":"t1","offset":[],"shape":[]}],"permute":[[0],[]],"scalars":[],"target":[[0],[]]}],"version":"0.0.1\n"}
        {
          ...
          for (size_t v5_batchId0 = (item.get_local_id(1) + item.get_group().get_local_range(1) * (item.get_group().get_group_id(2))); v5_batchId0 < numElements0; v5_batchId0 += (item.get_group_range(2) * item.get_group().get_local_range(1))) {
            ...
            if (allowed) {
              ...
              // r0 = abs(glb_m0)
              #pragma unroll
              for (int32_t v16_k0 = 0; v16_k0 < 2; ++v16_k0) {
                int32_t v17_lead = v16_k0 * 16;
                tensorforge::intel_esimd::simd<float, 16> v19_data;
                v19_data.copy_from(glb_m0 + (v17_lead));
                (tensorforge::intel_esimd::abs(v19_data)).copy_to(r0 + (v17_lead));
              }
              tensorforge::intel_esimd::simd<float, 8> v24_data;
              v24_data.copy_from(glb_m0 + (32_i32));
              (tensorforge::intel_esimd::abs(v24_data)).copy_to(r0 + (32));
              // s0 = store{r>s}(localShrMem0, r0);
              #pragma unroll
              for (int32_t v26_i0 = 0; v26_i0 < 2; ++v26_i0) {
                int32_t v27_a = v26_i0 * 16;
                tensorforge::intel_esimd::simd<float, 16> v28_data;
                v28_data.copy_from(r0 + (v27_a));
                tensorforge::slmStore<float, 16>(s0 + (v27_a), v28_data);
              }
              tensorforge::intel_esimd::simd<float, 8> v30_data;
              v30_data.copy_from(r0 + (32));
              tensorforge::slmStore<float, 8>(s0 + (32_i32), v30_data);
              float r1[1]{};
              // r1 = max(s0, dims=[0])
              tensorforge::intel_esimd::simd<float, 48> s0_run0 = tensorforge::slmLoad<float, 48>(s0 + (0_i32));
              tensorforge::intel_esimd::simd<float, 16> v38_data(s0_run0.template select<16, 1>(0));
              tensorforge::intel_esimd::simd<float, 16> v41_data(s0_run0.template select<16, 1>(16));
              tensorforge::intel_esimd::simd<float, 16> v46_data(s0_run0.template select<16, 1>(32));
              tensorforge::intel_esimd::simd<float, 16> v48_own(-INFINITY);
              v48_own.merge(tensorforge::intel_esimd::simd<float, 16>(v46_data), ((tensorforge::intel_esimd::simd<int32_t, 16>(0, 1)) < 8));
              r1[0] = (tensorforge::intel_esimd::hmax<float>((tensorforge::intel_esimd::max((tensorforge::intel_esimd::max(v38_data, v41_data)), v48_own))));
              // glb_m1 = mul(glb_m0, r1)
              float v55_data = r1[0];
              #pragma unroll
              for (int32_t v51_k0 = 0; v51_k0 < 2; ++v51_k0) {
                int32_t v52_lead = v51_k0 * 16;
                tensorforge::intel_esimd::simd<float, 16> v54_data;
                v54_data.copy_from(glb_m0 + (v52_lead));
                ((v54_data * v55_data)).copy_to(glb_m1 + (v52_lead));
              }
              tensorforge::intel_esimd::simd<float, 8> v57_data;
              v57_data.copy_from(glb_m0 + (32_i32));
              float v58_data = r1[0];
              ((v57_data * v58_data)).copy_to(glb_m1 + (32_i32));
            }
          }
        }
      }
