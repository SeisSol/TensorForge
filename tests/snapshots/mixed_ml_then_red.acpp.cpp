// === base name ===
kernel_c7283ba25cc7110f

// === header ===
void launcher_kernel_c7283ba25cc7110f(const float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 = nullptr, void* streamPtr = nullptr);


// === launcher ===
void launcher_kernel_c7283ba25cc7110f(const float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 , void* streamPtr) {
  ...
  kernel_kernel_c7283ba25cc7110f(stream, grid, block,  m0,  m0_extraOffset,  m1,  m1_extraOffset,  m2,  m2_extraOffset,  numElements0,  flags0 );
  CHECK_ERR;
}


// === kernel ===
inline void kernel_kernel_c7283ba25cc7110f(sycl::queue *stream, sycl::range<3> group_count, sycl::range<3> group_size, const float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 ) {
        ...
        // OUT = +(TMP, dims=[1])
        {
          ...
          for (size_t batchId0 = item.get_local_id(1) + item.get_group().get_local_range(1) * (item.get_group().get_group_id(0)); batchId0 < numElements0; batchId0 += (item.get_global_range(0) * item.get_group().get_local_range(1))) {
            ...
            if (allowed) {
              ...
              // glb_m2 = +(s0, dims=[1])
              if (v13_lead < 8) {
                float v479_acc0 = 0.0f;
                #pragma unroll
                for (int32_t v478_r1 = 0; v478_r1 < 8; ++v478_r1) {
                  int32_t v486_a = v13_lead + (v478_r1 * 8);
                  float v490_data = s0[(v486_a ^ ((v486_a >> 5) & 31))];
                  v479_acc0 = (v479_acc0 + v490_data);
                }
                glb_m2[v13_lead] = v479_acc0;
              }
              sycl::group_barrier(item.get_sub_group());
            }
          }
