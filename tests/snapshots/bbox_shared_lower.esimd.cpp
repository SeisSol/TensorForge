          ...
          if (batchId0 < numElements0) {
            ...
            if (allowed) {
              ...
              *(sycl::vec<float, 4>*)&s0[0 + 0 + 4 * item.get_local_id(0) + 64] = *(sycl::vec<float, 4>*)&glb_m2[0 + 0 + 4 * item.get_local_id(0) + 64];
              ...
              #pragma unroll
              for (int32_t v382_n1 = 0; v382_n1 < 8; ++v382_n1) {
                int32_t v384_a = 4 + (v382_n1 * 16);
                tensorforge::intel_esimd::simd<float, 12> v385_data;
                v385_data.copy_from(ir0 + (v384_a));
                v385_data.copy_to(r0 + (v384_a));
              }
              // glb_m0 = store{r>g}(r0);
              #pragma unroll
              for (int32_t v388_i1 = 0; v388_i1 < 8; ++v388_i1) {
                tensorforge::intel_esimd::simd<float, 12> v391_data;
                v391_data.copy_from(r0 + ((4 + (v388_i1 * 16))));
                v391_data.copy_to(glb_m0 + ((v388_i1 * 12)));
              }
            }
          }
        }
