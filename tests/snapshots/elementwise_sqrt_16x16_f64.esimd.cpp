// === base name ===
kernel_f0772cedd524bec4

// === header ===
void launcher_kernel_f0772cedd524bec4(const double* m0, size_t m0_extraOffset, double* m1, size_t m1_extraOffset, size_t numElements0, unsigned* flags0 = nullptr, void* streamPtr = nullptr);


// === launcher ===
void launcher_kernel_f0772cedd524bec4(const double* m0, size_t m0_extraOffset, double* m1, size_t m1_extraOffset, size_t numElements0, unsigned* flags0 , void* streamPtr) {
  ...
  kernel_kernel_f0772cedd524bec4(stream, grid, block,  m0,  m0_extraOffset,  m1,  m1_extraOffset,  numElements0,  flags0 );
  CHECK_ERR;
}


// === kernel ===
inline void kernel_kernel_f0772cedd524bec4(sycl::queue *stream, sycl::range<3> group_count, sycl::range<3> group_size, const double* m0, size_t m0_extraOffset, double* m1, size_t m1_extraOffset, size_t numElements0, unsigned* flags0 ) {
      ...
      cgh.parallel_for(sycl::nd_range<3>{{group_size.get(0), group_size.get(1), group_count.get(0) * group_size.get(2)}, group_size}, sycl::ext::oneapi::experimental::properties{sycl::ext::intel::experimental::grf_size<256>}, [=](sycl::nd_item<3> item) [[intel::sycl_explicit_simd]] [[intel::kernel_args_restrict]] {
        ...
        // B = sqrt(A)
        {
          ...
          for (size_t v2_batchId0 = (item.get_local_id(1) + item.get_group().get_local_range(1) * (item.get_group().get_group_id(0))); v2_batchId0 < numElements0; v2_batchId0 += (item.get_global_range(0) * item.get_group().get_local_range(1))) {
            ...
            if (allowed) {
              const double *const __restrict__ glb_m0 = &m0[v2_batchId0 * 256 + 0 + m0_extraOffset];
              double *const __restrict__ glb_m1 = &m1[v2_batchId0 * 256 + 0 + m1_extraOffset];
              // glb_m1 = sqrt(glb_m0)
              #pragma unroll
