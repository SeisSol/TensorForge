    ...
    // m0 12×8(12×8) {0..12}×{0..8} strided({0..12}×{0..8})[0, 1] += m1 12×16(12×16) {0..12}×{0..16} strided({0..12}×{0..16})[0, -1]×m2 16×8(16×8) {0..16}×{0..8} strided({0..16}×{0..8})[-1, 1]
    {
      ...
      for (size_t batchId0 = threadIdx.y + blockDim.y * (blockIdx.x); batchId0 < numElements0; batchId0 += (gridDim.x * blockDim.y)) {
        ...
        if (allowed) {
          ...
          ir2[7] = (v106_acc[3]);
          // glb_m0 = store{r>g}(r2);
          int32_t v113_lead = threadIdx.x % 16;
          if (v113_lead < 12) {
            #pragma unroll
            for (int32_t v115_i1 = 0; v115_i1 < 8; ++v115_i1) {
              int32_t v116_a = 0 + v115_i1;
              float v118_data = r2[v115_i1];
              int32_t v125_a = v113_lead + (v115_i1 * 12);
              __builtin_amdgcn_global_atomic_fadd_f32(&glb_m0[v125_a], v118_data);
            }
          }
          ;
        }
