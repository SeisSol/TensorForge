          ...
          if (batchId0 < numElements0) {
            ...
            if (allowed) {
              ...
              *(sycl::vec<float, 4>*)&s0[0 + 0 + 4 * item.get_local_id(0) + 64] = *(sycl::vec<float, 4>*)&glb_m2[0 + 0 + 4 * item.get_local_id(0) + 64];
              ...
              #pragma unroll
              for (int32_t v390_n1 = 0; v390_n1 < 8; ++v390_n1) {
                int32_t v391_a = v390_n1 * 16;
                tensorforge::intel_esimd::simd<float, 12> v393_data;
                v393_data.copy_from(ir0 + (v391_a));
                tensorforge::intel_esimd::simd<float, 12> v398_data;
                v398_data.copy_from(glb_m0 + ((v390_n1 * 12)));
                (v398_data + v393_data).copy_to(r0 + (v391_a));
              }
              // glb_m0 = store{r>g}(r0);
              #pragma unroll
              for (int32_t v402_i1 = 0; v402_i1 < 8; ++v402_i1) {
                tensorforge::intel_esimd::simd<float, 12> v405_data;
                v405_data.copy_from(r0 + ((v402_i1 * 16)));
                v405_data.copy_to(glb_m0 + ((v402_i1 * 12)));
              }
            }
          }
        }
