// === base name ===
kernel_fb68747bf12c2d8b

// === header ===
void launcher_kernel_fb68747bf12c2d8b(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 = nullptr, void* streamPtr = nullptr);


// === launcher ===
void launcher_kernel_fb68747bf12c2d8b(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 , void* streamPtr) {
  dim3 block (16, 8, 1);
  static std::size_t gridsize = 0;
      if (gridsize == 0) {
        ...
        CHECK_ERR;
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(&blocksPerSM, kernel_kernel_fb68747bf12c2d8b, block.x * block.y * block.z, 1152 * sizeof(float));
        CHECK_ERR;
        if (blocksPerSM > 0) {
  ...
  static bool shmemsizeset = false;
      if (!shmemsizeset) {
        cudaFuncSetAttribute(kernel_kernel_fb68747bf12c2d8b, cudaFuncAttributeMaxDynamicSharedMemorySize, 1152 * sizeof(float));
        CHECK_ERR;
        shmemsizeset = true;
      }
      
  cudaStream_t stream = (streamPtr != nullptr) ? static_cast<cudaStream_t>(streamPtr) : 0;
  kernel_kernel_fb68747bf12c2d8b<<<grid,block,1152 * sizeof(float),stream>>>( m0,  m0_extraOffset,  m1,  m1_extraOffset,  m2,  m2_extraOffset,  numElements0,  flags0 );
  CHECK_ERR;
}
...
__launch_bounds__(128)
 kernel_kernel_fb68747bf12c2d8b(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 ) {
      ...
      for (size_t v4_batchId0 = (threadIdx.y + blockDim.y * (blockIdx.x)); v4_batchId0 < numElements0; v4_batchId0 += (gridDim.x * blockDim.y)) {
        ...
        if (allowed) {
          ...
          for (int32_t v733_i0 = 0; v733_i0 < 1; ++v733_i0) {
            int32_t v741_lead = v18_lead + (v733_i0 * 16);
            #pragma unroll
            for (int32_t v734_i1 = 0; v734_i1 < 8; ++v734_i1) {
              float v736_data = r1[(v733_i0 + v734_i1)];
              glb_m0[(v741_lead + (v734_i1 * 16))] = v736_data;
            }
          }
          __syncwarp(0x0000ffffu << (threadIdx.y % 2 * 16));
        }
