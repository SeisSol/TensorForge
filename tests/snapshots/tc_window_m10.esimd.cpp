...
#endif
inline constexpr tensorforge::LaunchInfo launch_info_kernel_6bb3fee29dfdda24 = {{1, 16, 1}, 16, 10, 1, 16, 11264, false, true, 1};
tensorforge::LaunchConfig launch_config_kernel_6bb3fee29dfdda24(size_t numElements0, void* streamPtr = nullptr);
void launcher_kernel_6bb3fee29dfdda24(float * m0, size_t m0_extraOffset, const float * m1, const float * m2, size_t m2_extraOffset, size_t numElements0, unsigned * flags0 = nullptr, void* streamPtr = nullptr);
...
#endif
tensorforge::LaunchConfig launch_config_kernel_6bb3fee29dfdda24(size_t numElements0, void* streamPtr) {
  (void)numElements0;
  (void)streamPtr;
...
void launcher_kernel_6bb3fee29dfdda24(float * m0, size_t m0_extraOffset, const float * m1, const float * m2, size_t m2_extraOffset, size_t numElements0, unsigned * flags0, void* streamPtr) {
  const tensorforge::LaunchConfig config = launch_config_kernel_6bb3fee29dfdda24(numElements0, streamPtr);
  ...
  kernel_kernel_6bb3fee29dfdda24(stream, grid, block, m0, m0_extraOffset, m1, m2, m2_extraOffset, numElements0, flags0);
  CHECK_ERR;
}


// === kernel ===
inline void kernel_kernel_6bb3fee29dfdda24(sycl::queue *stream, sycl::range<3> group_count, sycl::range<3> group_size, float * m0, size_t m0_extraOffset, const float * m1, const float * m2, size_t m2_extraOffset, size_t numElements0, unsigned * flags0) {
          ...
          for (size_t v6_batchId0 = (item.get_local_id(1) + item.get_group().get_local_range(1) * (item.get_group().get_group_id(2))); v6_batchId0 < numElements0; v6_batchId0 += (item.get_group_range(2) * item.get_group().get_local_range(1))) {
            ...
            if (allowed) {
              ...
              for (int32_t v423_n1 = 0; v423_n1 < 9; ++v423_n1) {
                ...
                tensorforge::intel_esimd::simd<float, 10> v426_data(ir0.template select<10, 1>(v424_a));
                r0.template select<10, 1>(v424_a) = v426_data;
              }
              // glb_m0 = store{r>g}(r0);
              #pragma unroll
              for (int32_t v427_i1 = 0; v427_i1 < 9; ++v427_i1) {
                tensorforge::intel_esimd::simd<float, 10> v430_data(r0.template select<10, 1>((v427_i1 * 16)));
                v430_data.copy_to(glb_m0 + ((v427_i1 * 10)));
              }
            }
