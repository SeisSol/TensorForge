...
#endif
inline constexpr tensorforge::LaunchInfo launch_info_kernel_a69a55bb06cce0cd = {{32, 4, 1}, 32, 32, 1, 4, 0, false, true, 1};
tensorforge::LaunchConfig launch_config_kernel_a69a55bb06cce0cd(size_t numElements0, void* streamPtr = nullptr);
void launcher_kernel_a69a55bb06cce0cd(const float ** m0, size_t m0_extraOffset, const float ** m1, size_t m1_extraOffset, float ** m2, size_t m2_extraOffset, size_t numElements0, unsigned * flags0 = nullptr, void* streamPtr = nullptr);
...
#endif
tensorforge::LaunchConfig launch_config_kernel_a69a55bb06cce0cd(size_t numElements0, void* streamPtr) {
  (void)numElements0;
  (void)streamPtr;
  dim3 block (32, 4, 1);
  static std::size_t gridsize = 0;
      if (gridsize == 0) {
        ...
        CHECK_ERR;
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(&blocksPerSM, kernel_kernel_a69a55bb06cce0cd, block.x * block.y * block.z, 0 * sizeof(float));
        CHECK_ERR;
        if (blocksPerSM > 0) {
...
void launcher_kernel_a69a55bb06cce0cd(const float ** m0, size_t m0_extraOffset, const float ** m1, size_t m1_extraOffset, float ** m2, size_t m2_extraOffset, size_t numElements0, unsigned * flags0, void* streamPtr) {
  const tensorforge::LaunchConfig config = launch_config_kernel_a69a55bb06cce0cd(numElements0, streamPtr);
  dim3 block (config.block[0], config.block[1], config.block[2]);
  dim3 grid (config.grid[0], config.grid[1], config.grid[2]);
  static bool shmemsizeset = false;
      if (!shmemsizeset) {
        cudaFuncSetAttribute(kernel_kernel_a69a55bb06cce0cd, cudaFuncAttributeMaxDynamicSharedMemorySize, config.sharedMemBytes);
        CHECK_ERR;
        shmemsizeset = true;
      }
      
  cudaStream_t stream = (streamPtr != nullptr) ? static_cast<cudaStream_t>(streamPtr) : 0;
  kernel_kernel_a69a55bb06cce0cd<<<grid,block,config.sharedMemBytes,stream>>>(m0, m0_extraOffset, m1, m1_extraOffset, m2, m2_extraOffset, numElements0, flags0);
  CHECK_ERR;
}
...
__launch_bounds__(128)
 kernel_kernel_a69a55bb06cce0cd(const float ** m0, size_t m0_extraOffset, const float ** m1, size_t m1_extraOffset, float ** m2, size_t m2_extraOffset, size_t numElements0, unsigned * flags0) {
      ...
      for (size_t v1_batchId0 = (threadIdx.y + blockDim.y * (blockIdx.x)); v1_batchId0 < numElements0; v1_batchId0 += (gridDim.x * blockDim.y)) {
        ...
        if (allowed) {
          ...
          for (int32_t v16_i0 = 0; v16_i0 < 1; ++v16_i0) {
            float v19_data = __ldcg(&glb_m0[(v15_lead + (v16_i0 * 32))]);
            r0[v16_i0] = v19_data;
          }
          float r2[3]{};
          // r2 = load{g>r}(glb_m1);
          ...
          for (int32_t v21_i0 = 0; v21_i0 < 1; ++v21_i0) {
            ...
            for (int32_t v22_i1 = 0; v22_i1 < 3; ++v22_i1) {
              float v27_data = __ldcg(&glb_m1[(v24_lead + (v22_i1 * 32))]);
