        ...
        // C = abs(TMP)
        {
          ...
          if (batchId0 < numElements0) {
            ...
            if (allowed) {
              ...
              // glb_m2 = abs(s1)
              #pragma unroll
              for (int32_t v346_k1 = 0; v346_k1 < 8; ++v346_k1) {
                int32_t v349_a = v346_k1 * 8;
                tensorforge::intel_esimd::simd<float, 8> v351_data;
                v351_data.copy_from(s1 + (v349_a));
                (tensorforge::intel_esimd::abs(v351_data)).copy_to(glb_m2 + (v349_a));
              }
            }
          }
        }
