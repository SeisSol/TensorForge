// === base name ===
kernel_7470db6e1fc45c2d

// === header ===
void launcher_kernel_7470db6e1fc45c2d(const float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 = nullptr, void* streamPtr = nullptr);


// === launcher ===
void launcher_kernel_7470db6e1fc45c2d(const float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 , void* streamPtr) {
  dim3 block (32, 4, 1);
  static std::size_t gridsize = 0;
      if (gridsize == 0) {
        ...
        CHECK_ERR;
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(&blocksPerSM, kernel_kernel_7470db6e1fc45c2d, block.x * block.y * block.z, 256 * sizeof(float));
        CHECK_ERR;
        if (blocksPerSM > 0) {
  ...
  static bool shmemsizeset = false;
      if (!shmemsizeset) {
        cudaFuncSetAttribute(kernel_kernel_7470db6e1fc45c2d, cudaFuncAttributeMaxDynamicSharedMemorySize, 256 * sizeof(float));
        CHECK_ERR;
        shmemsizeset = true;
      }
      
  cudaStream_t stream = (streamPtr != nullptr) ? static_cast<cudaStream_t>(streamPtr) : 0;
  kernel_kernel_7470db6e1fc45c2d<<<grid,block,256 * sizeof(float),stream>>>( m0,  m0_extraOffset,  m1,  m1_extraOffset,  m2,  m2_extraOffset,  numElements0,  flags0 );
  CHECK_ERR;
}
...
__launch_bounds__(128)
 kernel_kernel_7470db6e1fc45c2d(const float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 ) {
    ...
    // OUT = +(TMP, dims=[1])
    {
      ...
      for (size_t v5_batchId0 = (threadIdx.y + blockDim.y * (blockIdx.x)); v5_batchId0 < numElements0; v5_batchId0 += (gridDim.x * blockDim.y)) {
        ...
        if (allowed) {
          ...
          float *const __restrict__ glb_m2 = &m2[v5_batchId0 * 8 + 0 + m2_extraOffset];
          float r0[8]{};
          // r0 = load{g>r}(glb_m0);
          ...
          // glb_m2 = +(s1, dims=[1])
          if (v19_lead < 8) {
            float v408_acc0 = 0.0f;
            #pragma unroll
            for (int32_t v407_r1 = 0; v407_r1 < 8; ++v407_r1) {
              int32_t v415_a = v19_lead + (v407_r1 * 8);
              float v419_data = s1[(v415_a ^ ((v415_a >> 5) & 31))];
              v408_acc0 = (v408_acc0 + v419_data);
            }
            glb_m2[v19_lead] = v408_acc0;
          }
          __syncwarp();
        }
      }
