// === base name ===
kernel_c8c611f7e5deda65

// === header ===
void launcher_kernel_c8c611f7e5deda65(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 = nullptr, void* streamPtr = nullptr);


// === launcher ===
void launcher_kernel_c8c611f7e5deda65(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 , void* streamPtr) {
  dim3 block (16, 8, 1);
  static std::size_t gridsize = 0;
      if (gridsize == 0) {
        ...
        CHECK_ERR;
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(&blocksPerSM, kernel_kernel_c8c611f7e5deda65, block.x * block.y * block.z, 1152 * sizeof(float));
        CHECK_ERR;
        if (blocksPerSM > 0) {
  ...
  static bool shmemsizeset = false;
      if (!shmemsizeset) {
        cudaFuncSetAttribute(kernel_kernel_c8c611f7e5deda65, cudaFuncAttributeMaxDynamicSharedMemorySize, 1152 * sizeof(float));
        CHECK_ERR;
        shmemsizeset = true;
      }
      
  cudaStream_t stream = (streamPtr != nullptr) ? static_cast<cudaStream_t>(streamPtr) : 0;
  kernel_kernel_c8c611f7e5deda65<<<grid,block,1152 * sizeof(float),stream>>>( m0,  m0_extraOffset,  m1,  m1_extraOffset,  m2,  m2_extraOffset,  numElements0,  flags0 );
  CHECK_ERR;
}
...
__launch_bounds__(128)
 kernel_kernel_c8c611f7e5deda65(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 ) {
        ...
        if (allowed) {
          ...
          if (v18_lead < 12) {
            ...
            for (int32_t v742_n1 = 0; v742_n1 < 8; ++v742_n1) {
              float v744_data = ir1[v742_n1];
              r1[v742_n1] = v744_data;
            }
          }
          // glb_m0 = store{r>g}(r1);
          if (v18_lead < 12) {
            ...
            for (int32_t v750_i1 = 0; v750_i1 < 8; ++v750_i1) {
              float v752_data = r1[v750_i1];
              glb_m0[(v18_lead + (v750_i1 * 12))] = v752_data;
            }
          }
          __syncwarp(0x0000ffffu << (threadIdx.y % 2 * 16));
        }
