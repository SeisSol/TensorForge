...
void launcher_kernel_9d96bbe702(const float* m0, unsigned m0_extraOffset, float* m1, unsigned m1_extraOffset, size_t numElements0, unsigned* flags0 , void* streamPtr) {
  ...
  kernel_kernel_9d96bbe702(stream, grid, block,  m0,  m0_extraOffset,  m1,  m1_extraOffset,  numElements0,  flags0 );
  CHECK_ERR;
}


// === kernel ===
inline void kernel_kernel_9d96bbe702(sycl::queue *stream, sycl::range<3> group_count, sycl::range<3> group_size, const float* m0, unsigned m0_extraOffset, float* m1, unsigned m1_extraOffset, size_t numElements0, unsigned* flags0 ) {
        ...
        // OUT = +(A, dims=[0])
        {
          ...
          if (batchId0 < numElements0) {
            ...
            if (allowed) {
              ...
              tensorforge::intel_esimd::simd<int32_t, 16> v5_lead = tensorforge::intel_esimd::simd<int32_t, 16>(0, 1);
              #pragma unroll
              for (int32_t v4_k1 = 0; v4_k1 < 16; ++v4_k1) {
                tensorforge::intel_esimd::simd<float, 16> v10_data;
                v10_data.copy_from(glb_m0 + ((v4_k1 * 16)));
                glb_m1[v4_k1] = (tensorforge::intel_esimd::reduce<float>(v10_data, std::plus<>()));
              }
            }
          }
        }
