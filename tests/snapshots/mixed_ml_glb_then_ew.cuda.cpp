// === base name ===
kernel_9cd63c067940e1b1

// === header ===
void launcher_kernel_9cd63c067940e1b1(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, float* m3, size_t m3_extraOffset, size_t numElements0, unsigned* flags0 = nullptr, void* streamPtr = nullptr);


// === launcher ===
void launcher_kernel_9cd63c067940e1b1(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, float* m3, size_t m3_extraOffset, size_t numElements0, unsigned* flags0 , void* streamPtr) {
  dim3 block (32, 4, 1);
  static std::size_t gridsize = 0;
      if (gridsize == 0) {
        ...
        CHECK_ERR;
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(&blocksPerSM, kernel_kernel_9cd63c067940e1b1, block.x * block.y * block.z, 256 * sizeof(float));
        CHECK_ERR;
        if (blocksPerSM > 0) {
  ...
  static bool shmemsizeset = false;
      if (!shmemsizeset) {
        cudaFuncSetAttribute(kernel_kernel_9cd63c067940e1b1, cudaFuncAttributeMaxDynamicSharedMemorySize, 256 * sizeof(float));
        CHECK_ERR;
        shmemsizeset = true;
      }
      
  cudaStream_t stream = (streamPtr != nullptr) ? static_cast<cudaStream_t>(streamPtr) : 0;
  kernel_kernel_9cd63c067940e1b1<<<grid,block,256 * sizeof(float),stream>>>( m0,  m0_extraOffset,  m1,  m1_extraOffset,  m2,  m2_extraOffset,  m3,  m3_extraOffset,  numElements0,  flags0 );
  CHECK_ERR;
}
...
__launch_bounds__(128)
 kernel_kernel_9cd63c067940e1b1(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, float* m3, size_t m3_extraOffset, size_t numElements0, unsigned* flags0 ) {
  extern __shared__ char totalShrMemPtr[];
   {
    ...
    // C = abs(M)
    {
      ...
      for (size_t v4_batchId0 = (threadIdx.y + blockDim.y * (blockIdx.x)); v4_batchId0 < numElements0; v4_batchId0 += (gridDim.x * blockDim.y)) {
        ...
        if (allowed) {
          float *const __restrict__ glb_m0 = &m0[v4_batchId0 * 64 + 0 + m0_extraOffset];
          ...
          if (v19_lead < 8) {
            ...
            for (int32_t v21_i1 = 0; v21_i1 < 8; ++v21_i1) {
              float v29_data = __ldcg(&glb_m1[(v19_lead + (v21_i1 * 8))]);
              r0[v21_i1] = v29_data;
            }
          }
          ...
          if (v19_lead < 8) {
            ...
            for (int32_t v399_i1 = 0; v399_i1 < 8; ++v399_i1) {
              ...
              glb_m0[(v19_lead + (v399_i1 * 8))] = v401_data;
            }
          }
          // glb_m3 = abs(glb_m0)
          if (v19_lead < 8) {
            #pragma unroll
            for (int32_t v413_k1 = 0; v413_k1 < 8; ++v413_k1) {
              int32_t v419_a = v413_k1 * 8;
              float v421_data = glb_m0[(v19_lead + v419_a)];
              glb_m3[(v19_lead + v419_a)] = (fabsf(v421_data));
            }
          }
          __syncwarp();
        }
