// === base name ===
kernel_65851997c6d17bba

// === header ===
#ifndef TENSORFORGE_LAUNCH_TYPES
#define TENSORFORGE_LAUNCH_TYPES
#include <cstddef>
namespace tensorforge {
// Fixed when the kernel is generated: `launch_info_<kernel>`.
struct LaunchInfo {
  unsigned block[3];
  unsigned threadsPerMult;
  unsigned activeThreads;
  unsigned leadWidth;
  unsigned multsPerBlock;
  std::size_t sharedMemBytes;
  bool cooperative;
  bool persistent;
  unsigned sections;
};
// What one launch uses, the grid included: `launch_config_<kernel>`.
struct LaunchConfig {
  std::size_t grid[3];
  std::size_t block[3];
  std::size_t sharedMemBytes;
  bool cooperative;
};
} // namespace tensorforge
#endif
inline constexpr tensorforge::LaunchInfo launch_info_kernel_65851997c6d17bba = {{1, 8, 1}, 32, 32, 1, 8, 5632, false, true, 1};
tensorforge::LaunchConfig launch_config_kernel_65851997c6d17bba(size_t numElements0, void* streamPtr = nullptr);
void launcher_kernel_65851997c6d17bba(float * m0, size_t m0_extraOffset, const float * m1, size_t m1_extraOffset, const float * m2, size_t m2_extraOffset, size_t numElements0, unsigned * flags0 = nullptr, void* streamPtr = nullptr);


// === launcher ===
#ifndef TENSORFORGE_LAUNCH_TYPES
#define TENSORFORGE_LAUNCH_TYPES
#include <cstddef>
namespace tensorforge {
// Fixed when the kernel is generated: `launch_info_<kernel>`.
struct LaunchInfo {
  unsigned block[3];
  unsigned threadsPerMult;
  unsigned activeThreads;
  unsigned leadWidth;
  unsigned multsPerBlock;
  std::size_t sharedMemBytes;
  bool cooperative;
  bool persistent;
  unsigned sections;
};
// What one launch uses, the grid included: `launch_config_<kernel>`.
struct LaunchConfig {
  std::size_t grid[3];
  std::size_t block[3];
  std::size_t sharedMemBytes;
  bool cooperative;
};
} // namespace tensorforge
#endif
tensorforge::LaunchConfig launch_config_kernel_65851997c6d17bba(size_t numElements0, void* streamPtr) {
  (void)numElements0;
  (void)streamPtr;
  sycl::range<3> block (1, 8, 1);
  tensorforge::LaunchConfig config{};
  config.grid[0] = (numElements0 + 8 - 1) / 8;
  config.grid[1] = 1;
  config.grid[2] = 1;
  config.block[0] = 1;
  config.block[1] = 8;
  config.block[2] = 1;
  config.sharedMemBytes = 1408 * sizeof(float);
  config.cooperative = false;
  return config;
}
void launcher_kernel_65851997c6d17bba(float * m0, size_t m0_extraOffset, const float * m1, size_t m1_extraOffset, const float * m2, size_t m2_extraOffset, size_t numElements0, unsigned * flags0, void* streamPtr) {
  const tensorforge::LaunchConfig config = launch_config_kernel_65851997c6d17bba(numElements0, streamPtr);
  sycl::range<3> block (config.block[0], config.block[1], config.block[2]);
  sycl::range<3> grid (config.grid[0], config.grid[1], config.grid[2]);
  if (streamPtr == nullptr) {
    throw std::invalid_argument("stream may not be null!");
  }
  sycl::queue *stream = static_cast<sycl::queue *>(streamPtr);
  kernel_kernel_65851997c6d17bba(stream, grid, block, m0, m0_extraOffset, m1, m1_extraOffset, m2, m2_extraOffset, numElements0, flags0);
  CHECK_ERR;
}


// === kernel ===
inline void kernel_kernel_65851997c6d17bba(sycl::queue *stream, sycl::range<3> group_count, sycl::range<3> group_size, float * m0, size_t m0_extraOffset, const float * m1, size_t m1_extraOffset, const float * m2, size_t m2_extraOffset, size_t numElements0, unsigned * flags0) {
  stream->submit([&](sycl::handler &cgh) {
    cgh.parallel_for(sycl::nd_range<3>{{group_count.get(2) * group_size.get(2), group_count.get(1) * group_size.get(1), group_count.get(0) * group_size.get(0)}, {group_size.get(2), group_size.get(1), group_size.get(0)}}, sycl::ext::oneapi::experimental::properties{sycl::ext::intel::experimental::grf_size<256>}, [=](sycl::nd_item<3> item) [[intel::sycl_explicit_simd]] [[intel::kernel_args_restrict]] {
      tensorforge::slmReserve<1408 * sizeof(float)>(); {
        // generated with TensorForge. Version: 0.0.1
        // options: default
        // launch: 32 lanes x 8 per block = block 1x8x1, 5632 B shared, occupancy grid
        // operands:
        //   m0 32×13(32×13) {0..32}×{0..13} strided
        //   m1 32×13(32×13) {0..32}×{0..13} strided
        //   m2 13×13(13×13) {0..13}×{0..13} strided
        // operations:
        //   m0[i,j]@{0..32}×{6..13} = m1[i,k]@{0..32}×{10..13} × m2[k,j]@{10..13}×{6..13}
        // tensorforge-meta: {"fp":"float","launch":{"active_threads":32,"block":[1,8,1],"cooperative":false,"lead_width":1,"mults_per_block":8,"persistent":true,"sections":[{"barrier":false,"mults_per_block":8,"shared_elements":1408}],"shared_bytes":5632,"shared_elements":1408,"threads_per_mult":32},"loops":[],"operands":[{"addressing":"strided","alias":"D","bbox":[[0,0],[32,13]],"name":"m0","ordered":false,"parts":1,"shape":[32,13],"variant":false},{"addressing":"strided","alias":"Q","bbox":[[0,0],[32,13]],"name":"m1","ordered":false,"parts":1,"shape":[32,13],"variant":false},{"addressing":"strided","alias":"S","bbox":[[0,0],[13,13]],"name":"m2","ordered":false,"parts":1,"shape":[13,13],"variant":false}],"operations":[{"add":false,"dest":{"addressing":"strided","bbox":[[0,6],[32,13]],"is_tmp":false,"name":"m0","offset":[0,0],"shape":[32,13]},"kind":"multilinear","ops":[{"addressing":"strided","bbox":[[0,10],[32,13]],"is_tmp":false,"name":"m1","offset":[0,0],"shape":[32,13]},{"addressing":"strided","bbox":[[10,6],[13,13]],"is_tmp":false,"name":"m2","offset":[0,0],"shape":[13,13]}],"permute":[[0,1],[0,1]],"target":[[0,-1],[-1,1]]}],"version":"0.0.1\n"}
        {
          const auto batchId_start = (item.get_local_id(1) + item.get_group().get_local_range(1) * (item.get_group().get_group_id(2)));
          const auto batchId1 = batchId_start < numElements0 ? batchId_start : 0;
          const auto batchId2 = batchId1 + (item.get_group_range(2) * item.get_group().get_local_range(1)) < numElements0 ? batchId1 + (item.get_group_range(2) * item.get_group().get_local_range(1)) : batchId1;
          tensorforge::SlmPtr<float> totalShrMem = tensorforge::SlmPtr<float>(0);
          tensorforge::SlmPtr<float> localShrMem0 = totalShrMem + (176 * item.get_local_id(1) + 0);
          tensorforge::SlmPtr<float> tempShrMem = localShrMem0 + (176);
          tensorforge::SlmPtr<float> s0 = localShrMem0 + (0);
          for (size_t v5_batchId0 = (item.get_local_id(1) + item.get_group().get_local_range(1) * (item.get_group().get_group_id(2))); v5_batchId0 < numElements0; v5_batchId0 += (item.get_group_range(2) * item.get_group().get_local_range(1))) {
            size_t v6_ahead1 = v5_batchId0 + (item.get_group_range(2) * item.get_group().get_local_range(1));
            size_t v8_batchId1 = (v6_ahead1 < numElements0) ? v6_ahead1 : v5_batchId0;
            const bool allowed = flags0 == nullptr ? true : static_cast<bool>(flags0[v5_batchId0]);
            if (allowed) {
              float *const __restrict__ glb_m0 = &m0[v5_batchId0 * 416 + 0 + m0_extraOffset];
              const float *const __restrict__ glb_m1 = &m1[v5_batchId0 * 416 + 0 + m1_extraOffset];
              const float *const __restrict__ glb_m2 = &m2[v5_batchId0 * 169 + 0 + m2_extraOffset];
              tensorforge::intel_esimd::simd<float, 96> r0(0.0f);
              // r0 = load{g>r}(glb_m1);
              #pragma unroll
              for (int32_t v17_i0 = 0; v17_i0 < 1; ++v17_i0) {
                int32_t v19_lead = v17_i0 * 32;
                #pragma unroll
                for (int32_t v18_i1 = 10; v18_i1 < 13; ++v18_i1) {
                  tensorforge::intel_esimd::simd<float, 32> v23_data;
                  v23_data.copy_from(glb_m1 + ((v19_lead + (v18_i1 * 32))));
                  r0.template select<32, 1>((v19_lead + ((v18_i1 - 10) * 32))) = v23_data;
                }
              }
              // s0 = load{g>s}(glb_m2[0, 1])
              tensorforge::intel_esimd::simd<float, 128> v27_ld;
              v27_ld.copy_from(glb_m2 + (0 + 0 + 4 * 0 + 0));
              tensorforge::slmStore<float, 128>(s0 + (0 + 0 + 4 * 0 + 0), v27_ld);
              tensorforge::intel_esimd::simd<float, 32> v28_ld;
              v28_ld.copy_from(glb_m2 + (0 + 0 + 1 * 0 + 128));
              tensorforge::slmStore<float, 32>(s0 + (0 + 0 + 1 * 0 + 128), v28_ld);
              tensorforge::intel_esimd::simd<float, 9> v29_ld;
              v29_ld.copy_from(glb_m2 + (0 + 0 + 1 * 0 + 160));
              tensorforge::slmStore<float, 9>(s0 + (0 + 0 + 1 * 0 + 160), v29_ld);
              // wait(r0 = load{g>r}(glb_m1););
              // wait(s0 = load{g>s}(glb_m2[0, 1]));
              tensorforge::intel_esimd::simd<float, 224> r1(0.0f);
              // ir1 = +(r0 * s0)
              // [(0, 32), (6, 13)] [(10, 13)]
              tensorforge::intel_esimd::simd<float, 224> ir1(0.0f);
              tensorforge::intel_esimd::simd<float, 32> v32_data(r0.template select<32, 1>(0));
              tensorforge::intel_esimd::simd<float, 88> s0_w0 = tensorforge::slmLoad<float, 88>(s0 + 88);
              float v33_data = s0_w0[0];
              tensorforge::intel_esimd::simd<float, 32> v35_data(ir1.template select<32, 1>(0));
              ir1.template select<32, 1>(0) = (v35_data + (v32_data * v33_data));
              float v38_data = s0_w0[13];
              tensorforge::intel_esimd::simd<float, 32> v40_data(ir1.template select<32, 1>(32));
              ir1.template select<32, 1>(32) = (v40_data + (v32_data * v38_data));
              float v43_data = s0_w0[26];
              tensorforge::intel_esimd::simd<float, 32> v45_data(ir1.template select<32, 1>(64));
              ir1.template select<32, 1>(64) = (v45_data + (v32_data * v43_data));
              float v48_data = s0_w0[39];
              tensorforge::intel_esimd::simd<float, 32> v50_data(ir1.template select<32, 1>(96));
              ir1.template select<32, 1>(96) = (v50_data + (v32_data * v48_data));
              float v53_data = s0_w0[52];
              tensorforge::intel_esimd::simd<float, 32> v55_data(ir1.template select<32, 1>(128));
              ir1.template select<32, 1>(128) = (v55_data + (v32_data * v53_data));
              float v58_data = s0_w0[65];
              tensorforge::intel_esimd::simd<float, 32> v60_data(ir1.template select<32, 1>(160));
              ir1.template select<32, 1>(160) = (v60_data + (v32_data * v58_data));
              float v63_data = s0_w0[78];
              tensorforge::intel_esimd::simd<float, 32> v65_data(ir1.template select<32, 1>(192));
              ir1.template select<32, 1>(192) = (v65_data + (v32_data * v63_data));
              tensorforge::intel_esimd::simd<float, 32> v67_data(r0.template select<32, 1>(32));
              float v68_data = s0_w0[1];
              tensorforge::intel_esimd::simd<float, 32> v70_data(ir1.template select<32, 1>(0));
              ir1.template select<32, 1>(0) = (v70_data + (v67_data * v68_data));
              float v73_data = s0_w0[14];
              tensorforge::intel_esimd::simd<float, 32> v75_data(ir1.template select<32, 1>(32));
              ir1.template select<32, 1>(32) = (v75_data + (v67_data * v73_data));
              float v78_data = s0_w0[27];
              tensorforge::intel_esimd::simd<float, 32> v80_data(ir1.template select<32, 1>(64));
              ir1.template select<32, 1>(64) = (v80_data + (v67_data * v78_data));
              float v83_data = s0_w0[40];
              tensorforge::intel_esimd::simd<float, 32> v85_data(ir1.template select<32, 1>(96));
              ir1.template select<32, 1>(96) = (v85_data + (v67_data * v83_data));
              float v88_data = s0_w0[53];
              tensorforge::intel_esimd::simd<float, 32> v90_data(ir1.template select<32, 1>(128));
              ir1.template select<32, 1>(128) = (v90_data + (v67_data * v88_data));
              float v93_data = s0_w0[66];
              tensorforge::intel_esimd::simd<float, 32> v95_data(ir1.template select<32, 1>(160));
              ir1.template select<32, 1>(160) = (v95_data + (v67_data * v93_data));
              float v98_data = s0_w0[79];
              tensorforge::intel_esimd::simd<float, 32> v100_data(ir1.template select<32, 1>(192));
              ir1.template select<32, 1>(192) = (v100_data + (v67_data * v98_data));
              tensorforge::intel_esimd::simd<float, 32> v102_data(r0.template select<32, 1>(64));
              float v103_data = s0_w0[2];
              tensorforge::intel_esimd::simd<float, 32> v105_data(ir1.template select<32, 1>(0));
              ir1.template select<32, 1>(0) = (v105_data + (v102_data * v103_data));
              float v108_data = s0_w0[15];
              tensorforge::intel_esimd::simd<float, 32> v110_data(ir1.template select<32, 1>(32));
              ir1.template select<32, 1>(32) = (v110_data + (v102_data * v108_data));
              float v113_data = s0_w0[28];
              tensorforge::intel_esimd::simd<float, 32> v115_data(ir1.template select<32, 1>(64));
              ir1.template select<32, 1>(64) = (v115_data + (v102_data * v113_data));
              float v118_data = s0_w0[41];
              tensorforge::intel_esimd::simd<float, 32> v120_data(ir1.template select<32, 1>(96));
              ir1.template select<32, 1>(96) = (v120_data + (v102_data * v118_data));
              float v123_data = s0_w0[54];
              tensorforge::intel_esimd::simd<float, 32> v125_data(ir1.template select<32, 1>(128));
              ir1.template select<32, 1>(128) = (v125_data + (v102_data * v123_data));
              float v128_data = s0_w0[67];
              tensorforge::intel_esimd::simd<float, 32> v130_data(ir1.template select<32, 1>(160));
              ir1.template select<32, 1>(160) = (v130_data + (v102_data * v128_data));
              float v133_data = s0_w0[80];
              tensorforge::intel_esimd::simd<float, 32> v135_data(ir1.template select<32, 1>(192));
              ir1.template select<32, 1>(192) = (v135_data + (v102_data * v133_data));
              // r1 = ir1
              #pragma unroll
              for (int32_t v137_n0 = 0; v137_n0 < 1; ++v137_n0) {
                int32_t v139_a = v137_n0 * 32;
                #pragma unroll
                for (int32_t v138_n1 = 6; v138_n1 < 13; ++v138_n1) {
                  int32_t v142_a = v139_a + ((v138_n1 - 6) * 32);
                  tensorforge::intel_esimd::simd<float, 32> v143_data(ir1.template select<32, 1>(v142_a));
                  r1.template select<32, 1>(v142_a) = v143_data;
                }
              }
              // glb_m0 = store{r>g}(r1);
              #pragma unroll
              for (int32_t v144_i0 = 0; v144_i0 < 1; ++v144_i0) {
                int32_t v146_lead = v144_i0 * 32;
                glb_m0[v146_lead] = 0.0f;
                int32_t v149_a = v146_lead + 32;
                glb_m0[v149_a] = 0.0f;
                int32_t v150_a = v146_lead + 64;
                glb_m0[v150_a] = 0.0f;
                int32_t v151_a = v146_lead + 96;
                glb_m0[v151_a] = 0.0f;
                int32_t v152_a = v146_lead + 128;
                glb_m0[v152_a] = 0.0f;
                int32_t v153_a = v146_lead + 160;
                glb_m0[v153_a] = 0.0f;
                tensorforge::intel_esimd::simd<float, 32> v155_data(r1.template select<32, 1>(v146_lead));
                int32_t v156_a = v146_lead + 192;
                v155_data.copy_to(glb_m0 + (v156_a));
                tensorforge::intel_esimd::simd<float, 32> v158_data(r1.template select<32, 1>(v149_a));
                v158_data.copy_to(glb_m0 + ((v146_lead + 224)));
                tensorforge::intel_esimd::simd<float, 32> v161_data(r1.template select<32, 1>(v150_a));
                v161_data.copy_to(glb_m0 + ((v146_lead + 256)));
                tensorforge::intel_esimd::simd<float, 32> v164_data(r1.template select<32, 1>(v151_a));
                v164_data.copy_to(glb_m0 + ((v146_lead + 288)));
                tensorforge::intel_esimd::simd<float, 32> v167_data(r1.template select<32, 1>(v152_a));
                v167_data.copy_to(glb_m0 + ((v146_lead + 320)));
                tensorforge::intel_esimd::simd<float, 32> v170_data(r1.template select<32, 1>(v153_a));
                v170_data.copy_to(glb_m0 + ((v146_lead + 352)));
                tensorforge::intel_esimd::simd<float, 32> v173_data(r1.template select<32, 1>(v156_a));
                v173_data.copy_to(glb_m0 + ((v146_lead + 384)));
              }
            }
          }
        }
      }
    });
  });
}

