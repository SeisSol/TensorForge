// === base name ===
kernel_26b44b481857e7fc

// === header ===
void launcher_kernel_26b44b481857e7fc(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 = nullptr, void* streamPtr = nullptr);


// === launcher ===
void launcher_kernel_26b44b481857e7fc(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 , void* streamPtr) {
  ...
  kernel_kernel_26b44b481857e7fc(stream, grid, block,  m0,  m0_extraOffset,  m1,  m1_extraOffset,  m2,  m2_extraOffset,  numElements0,  flags0 );
  CHECK_ERR;
}


// === kernel ===
inline void kernel_kernel_26b44b481857e7fc(sycl::queue *stream, sycl::range<3> group_count, sycl::range<3> group_size, float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 ) {
        ...
        // m0 16×11(16×11) {0..16}×{0..11} strided({0..16}×{0..11})[0, 1] = m1 16×16(16×16) {0..16}×{0..16} strided({0..16}×{0..16})[0, -1]×m2 16×11(16×11) {0..16}×{0..11} strided({0..16}×{0..11})[-1, 1]
        {
          ...
          for (size_t batchId0 = item.get_local_id(1) + item.get_group().get_local_range(1) * (item.get_group().get_group_id(0)); batchId0 < numElements0; batchId0 += (item.get_global_range(0) * item.get_group().get_local_range(1))) {
            ...
            if (allowed) {
              ...
              v23_ld.copy_from(glb_m2 + (0 + 0 + 2 * item.get_local_id(0) + 128));
              v23_ld.copy_to(s0 + (0 + 0 + 2 * item.get_local_id(0) + 128));
              ...
              #pragma unroll
              for (int32_t v450_n0 = 0; v450_n0 < 1; ++v450_n0) {
                int32_t v452_a = v450_n0 * 16;
                #pragma unroll
                for (int32_t v451_n1 = 0; v451_n1 < 11; ++v451_n1) {
                  int32_t v454_a = v452_a + (v451_n1 * 16);
                  tensorforge::intel_esimd::simd<float, 16> v455_data;
                  v455_data.copy_from(ir1 + (v454_a));
                  v455_data.copy_to(r1 + (v454_a));
                }
              }
              // glb_m0 = store{r>g}(r1);
              #pragma unroll
              for (int32_t v459_i0 = 0; v459_i0 < 1; ++v459_i0) {
                int32_t v461_a = v459_i0 * 16;
                #pragma unroll
                for (int32_t v460_i1 = 0; v460_i1 < 11; ++v460_i1) {
                  int32_t v463_a = v461_a + (v460_i1 * 16);
                  tensorforge::intel_esimd::simd<float, 16> v464_data;
                  v464_data.copy_from(r1 + (v463_a));
                  v464_data.copy_to(glb_m0 + (v463_a));
                }
              }
            }
          }
