// === base name ===
kernel_ea96fbb90d8fe810

// === header ===
void launcher_kernel_ea96fbb90d8fe810(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, float* m3, size_t m3_extraOffset, size_t numElements0, unsigned* flags0 = nullptr, void* streamPtr = nullptr);


// === launcher ===
void launcher_kernel_ea96fbb90d8fe810(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, float* m3, size_t m3_extraOffset, size_t numElements0, unsigned* flags0 , void* streamPtr) {
  ...
  kernel_kernel_ea96fbb90d8fe810(stream, grid, block,  m0,  m0_extraOffset,  m1,  m1_extraOffset,  m2,  m2_extraOffset,  m3,  m3_extraOffset,  numElements0,  flags0 );
  CHECK_ERR;
}


// === kernel ===
inline void kernel_kernel_ea96fbb90d8fe810(sycl::queue *stream, sycl::range<3> group_count, sycl::range<3> group_size, float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, float* m3, size_t m3_extraOffset, size_t numElements0, unsigned* flags0 ) {
          ...
          for (size_t batchId0 = item.get_local_id(1) + item.get_group().get_local_range(1) * (item.get_group().get_group_id(0)); batchId0 < numElements0; batchId0 += (item.get_global_range(0) * item.get_group().get_local_range(1))) {
            ...
            if (allowed) {
              ...
              for (int32_t v12_i1 = 0; v12_i1 < 8; ++v12_i1) {
                ...
                v17_data.copy_from(glb_m1 + ((v12_i1 * 8)));
              ...
              for (int32_t v205_i1 = 0; v205_i1 < 8; ++v205_i1) {
                ...
                v208_data.copy_to(glb_m0 + ((v205_i1 * 8)));
              }
              // glb_m3 = abs(glb_m0)
              #pragma unroll
              for (int32_t v213_k1 = 0; v213_k1 < 8; ++v213_k1) {
                int32_t v216_a = v213_k1 * 8;
                tensorforge::intel_esimd::simd<float, 8> v218_data;
                v218_data.copy_from(glb_m0 + (v216_a));
                (tensorforge::intel_esimd::abs(v218_data)).copy_to(glb_m3 + (v216_a));
              }
            }
          }
        }
