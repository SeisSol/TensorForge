// === base name ===
kernel_56b016958cf82ab9

// === header ===
void launcher_kernel_56b016958cf82ab9(const float* m0, size_t m0_extraOffset, float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 = nullptr, void* streamPtr = nullptr);


// === launcher ===
void launcher_kernel_56b016958cf82ab9(const float* m0, size_t m0_extraOffset, float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 , void* streamPtr) {
  ...
  kernel_kernel_56b016958cf82ab9(stream, grid, block,  m0,  m0_extraOffset,  m1,  m1_extraOffset,  m2,  m2_extraOffset,  numElements0,  flags0 );
  CHECK_ERR;
}


// === kernel ===
inline void kernel_kernel_56b016958cf82ab9(sycl::queue *stream, sycl::range<3> group_count, sycl::range<3> group_size, const float* m0, size_t m0_extraOffset, float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 ) {
          ...
          for (size_t v2_batchId0 = (item.get_local_id(1) + item.get_group().get_local_range(1) * (item.get_group().get_group_id(0))); v2_batchId0 < numElements0; v2_batchId0 += (item.get_global_range(0) * item.get_group().get_local_range(1))) {
            ...
            if (allowed) {
              ...
              for (int32_t v14_i1 = 0; v14_i1 < 8; ++v14_i1) {
                ...
                v19_data.copy_from(glb_m2 + ((v14_i1 * 8)));
                v19_data.copy_to(r1 + ((v14_i1 * 16)));
              }
              float r0[16]{};
              // r0 = +(glb_m0, dims=[1])
              tensorforge::intel_esimd::simd<float, 8> v24_acc0(0.0f);
              #pragma unroll
              for (int32_t v23_r1 = 0; v23_r1 < 8; ++v23_r1) {
                tensorforge::intel_esimd::simd<float, 8> v29_data;
                v29_data.copy_from(glb_m0 + ((v23_r1 * 8)));
                v24_acc0 = (v24_acc0 + v29_data);
              }
              v24_acc0.copy_to(r0 + (0));
              ...
              v36_data.copy_from(r1 + (0));
              v35_acc += ((static_cast<float>(v36_data[0])) * v34_data);
              v35_acc.copy_to(ir2 + (0));
              tensorforge::intel_esimd::simd<float, 16> v39_acc{};
              tensorforge::intel_esimd::simd<float, 16> v40_data;
              v40_data.copy_from(r1 + (16));
              v39_acc += ((static_cast<float>(v40_data[0])) * v34_data);
              v39_acc.copy_to(ir2 + (16));
              tensorforge::intel_esimd::simd<float, 16> v43_acc{};
              tensorforge::intel_esimd::simd<float, 16> v44_data;
              v44_data.copy_from(r1 + (32));
              v43_acc += ((static_cast<float>(v44_data[0])) * v34_data);
              v43_acc.copy_to(ir2 + (32));
              tensorforge::intel_esimd::simd<float, 16> v47_acc{};
              tensorforge::intel_esimd::simd<float, 16> v48_data;
              v48_data.copy_from(r1 + (48));
              v47_acc += ((static_cast<float>(v48_data[0])) * v34_data);
              v47_acc.copy_to(ir2 + (48));
              tensorforge::intel_esimd::simd<float, 16> v51_acc{};
              tensorforge::intel_esimd::simd<float, 16> v52_data;
              v52_data.copy_from(r1 + (64));
              v51_acc += ((static_cast<float>(v52_data[0])) * v34_data);
              v51_acc.copy_to(ir2 + (64));
              tensorforge::intel_esimd::simd<float, 16> v55_acc{};
              tensorforge::intel_esimd::simd<float, 16> v56_data;
              v56_data.copy_from(r1 + (80));
              v55_acc += ((static_cast<float>(v56_data[0])) * v34_data);
              v55_acc.copy_to(ir2 + (80));
              tensorforge::intel_esimd::simd<float, 16> v59_acc{};
              tensorforge::intel_esimd::simd<float, 16> v60_data;
              v60_data.copy_from(r1 + (96));
              v59_acc += ((static_cast<float>(v60_data[0])) * v34_data);
              v59_acc.copy_to(ir2 + (96));
              tensorforge::intel_esimd::simd<float, 16> v63_acc{};
              tensorforge::intel_esimd::simd<float, 16> v64_data;
              v64_data.copy_from(r1 + (112));
              v63_acc += ((static_cast<float>(v64_data[0])) * v34_data);
              v63_acc.copy_to(ir2 + (112));
              #pragma unroll
              for (int32_t v67_n1 = 0; v67_n1 < 8; ++v67_n1) {
                int32_t v68_a = v67_n1 * 16;
                tensorforge::intel_esimd::simd<float, 8> v70_data;
                v70_data.copy_from(ir2 + (v68_a));
                v70_data.copy_to(r2 + (v68_a));
              }
              // glb_m1 = store{r>g}(r2);
              #pragma unroll
              for (int32_t v73_i1 = 0; v73_i1 < 8; ++v73_i1) {
                tensorforge::intel_esimd::simd<float, 8> v76_data;
                v76_data.copy_from(r2 + ((v73_i1 * 16)));
                v76_data.copy_to(glb_m1 + ((v73_i1 * 8)));
              }
            }
          }
        }
