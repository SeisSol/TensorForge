...
void launcher_kernel_90d491a14e(const float* m0, unsigned m0_extraOffset, float* m1, unsigned m1_extraOffset, size_t numElements0, unsigned* flags0 , void* streamPtr) {
  ...
  static std::size_t gridsize = 0;
      if (gridsize == 0) {
        ...
        CHECK_ERR;
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(&blocksPerSM, kernel_kernel_90d491a14e, block.x * block.y * block.z, 0 * sizeof(float));
        CHECK_ERR;
        if (blocksPerSM > 0) {
  ...
  static bool shmemsizeset = false;
      if (!shmemsizeset) {
        cudaFuncSetAttribute(kernel_kernel_90d491a14e, cudaFuncAttributeMaxDynamicSharedMemorySize, 0 * sizeof(float));
        CHECK_ERR;
        shmemsizeset = true;
      }
      
  cudaStream_t stream = (streamPtr != nullptr) ? static_cast<cudaStream_t>(streamPtr) : 0;
  kernel_kernel_90d491a14e<<<grid,block,0 * sizeof(float),stream>>>( m0,  m0_extraOffset,  m1,  m1_extraOffset,  numElements0,  flags0 );
  CHECK_ERR;
}
        ...
        if (allowed) {
          ...
          if (v7_lead < 16) {
            ...
            for (int32_t v9_r1 = 0; v9_r1 < 16; ++v9_r1) {
              float v18_data = glb_m0[(v7_lead + (v9_r1 * 16))];
              v10_acc0 = (fmaxf(v10_acc0, v18_data));
            }
            v22_sel0 = v10_acc0;
          }
          else {
          ...
          float v23_red = tensorforge::reduction<tensorforge::ReductionOperation<float, tensorforge::Operation::Max>, 16, 1, float>(v22_sel0);
          if (v7_lead == 0) {
            glb_m1[0] = v23_red;
          }
          __syncwarp();
