// === base name ===
kernel_71dce481d4143283

// === header ===
void launcher_kernel_71dce481d4143283(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 = nullptr, void* streamPtr = nullptr);


// === launcher ===
void launcher_kernel_71dce481d4143283(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 , void* streamPtr) {
  ...
  kernel_kernel_71dce481d4143283(stream, grid, block,  m0,  m0_extraOffset,  m1,  m1_extraOffset,  m2,  m2_extraOffset,  numElements0,  flags0 );
  CHECK_ERR;
}


// === kernel ===
inline void kernel_kernel_71dce481d4143283(sycl::queue *stream, sycl::range<3> group_count, sycl::range<3> group_size, float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 ) {
        ...
        // m0 16×8(16×8) {0..16}×{0..8} strided({0..16}×{0..8})[0, 1] = m1 16×16(16×16) {0..16}×{0..16} strided({0..16}×{0..16})[0, -1]×m2 16×8(16×8) {0..16}×{0..8} strided({0..16}×{0..8})[-1, 1]
        {
          ...
          for (size_t batchId0 = item.get_local_id(1) + item.get_group().get_local_range(1) * (item.get_group().get_group_id(0)); batchId0 < numElements0; batchId0 += (item.get_global_range(0) * item.get_group().get_local_range(1))) {
            ...
            if (allowed) {
              ...
              #pragma unroll
              for (int32_t v337_n0 = 0; v337_n0 < 1; ++v337_n0) {
                int32_t v339_a = v337_n0 * 16;
                #pragma unroll
                for (int32_t v338_n1 = 0; v338_n1 < 8; ++v338_n1) {
                  int32_t v341_a = v339_a + (v338_n1 * 16);
                  tensorforge::intel_esimd::simd<float, 16> v342_data;
                  v342_data.copy_from(ir1 + (v341_a));
                  v342_data.copy_to(r1 + (v341_a));
                }
              }
              // glb_m0 = store{r>g}(r1);
              #pragma unroll
              for (int32_t v346_i0 = 0; v346_i0 < 1; ++v346_i0) {
                int32_t v348_a = v346_i0 * 16;
                #pragma unroll
                for (int32_t v347_i1 = 0; v347_i1 < 8; ++v347_i1) {
                  int32_t v350_a = v348_a + (v347_i1 * 16);
                  tensorforge::intel_esimd::simd<float, 16> v351_data;
                  v351_data.copy_from(r1 + (v350_a));
                  v351_data.copy_to(glb_m0 + (v350_a));
                }
              }
            }
          }
