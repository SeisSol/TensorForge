// === base name ===
kernel_01992e358dcf7f1b

// === header ===
void launcher_kernel_01992e358dcf7f1b(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 = nullptr, void* streamPtr = nullptr);


// === launcher ===
void launcher_kernel_01992e358dcf7f1b(float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 , void* streamPtr) {
  ...
  kernel_kernel_01992e358dcf7f1b(stream, grid, block,  m0,  m0_extraOffset,  m1,  m1_extraOffset,  m2,  m2_extraOffset,  numElements0,  flags0 );
  CHECK_ERR;
}


// === kernel ===
inline void kernel_kernel_01992e358dcf7f1b(sycl::queue *stream, sycl::range<3> group_count, sycl::range<3> group_size, float* m0, size_t m0_extraOffset, const float* m1, size_t m1_extraOffset, const float* m2, size_t m2_extraOffset, size_t numElements0, unsigned* flags0 ) {
        ...
        // m0 16×8(16×8) {0..16}×{0..8} strided({0..16}×{0..8})[0, 1] = m1 16×16(16×16) {0..16}×{0..16} strided({0..16}×{0..16})[0, -1]×m2 16×8(16×8) {0..16}×{0..8} strided({0..16}×{0..8})[-1, 1]
        {
          ...
          for (size_t v3_batchId0 = (item.get_local_id(1) + item.get_group().get_local_range(1) * (item.get_group().get_group_id(0))); v3_batchId0 < numElements0; v3_batchId0 += (item.get_global_range(0) * item.get_group().get_local_range(1))) {
            ...
            if (allowed) {
              ...
              #pragma unroll
              for (int32_t v341_n0 = 0; v341_n0 < 1; ++v341_n0) {
                int32_t v343_a = v341_n0 * 16;
                #pragma unroll
                for (int32_t v342_n1 = 0; v342_n1 < 8; ++v342_n1) {
                  int32_t v345_a = v343_a + (v342_n1 * 16);
                  tensorforge::intel_esimd::simd<float, 16> v346_data;
                  v346_data.copy_from(ir1 + (v345_a));
                  v346_data.copy_to(r1 + (v345_a));
                }
              }
              // glb_m0 = store{r>g}(r1);
              #pragma unroll
              for (int32_t v350_i0 = 0; v350_i0 < 1; ++v350_i0) {
                int32_t v352_a = v350_i0 * 16;
                #pragma unroll
                for (int32_t v351_i1 = 0; v351_i1 < 8; ++v351_i1) {
                  int32_t v354_a = v352_a + (v351_i1 * 16);
                  tensorforge::intel_esimd::simd<float, 16> v355_data;
                  v355_data.copy_from(r1 + (v354_a));
                  v355_data.copy_to(glb_m0 + (v354_a));
                }
              }
            }
          }
