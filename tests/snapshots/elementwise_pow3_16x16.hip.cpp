...
void launcher_kernel_9656a8bcfa(const float* m0, unsigned m0_extraOffset, float* m1, unsigned m1_extraOffset, size_t numElements0, unsigned* flags0 , void* streamPtr) {
  ...
  static std::size_t gridsize = 0;
      if (gridsize == 0) {
        ...
        CHECK_RES(hipOccupancyMaxActiveBlocksPerMultiprocessor(&blocksPerSM, kernel_kernel_9656a8bcfa, block.x * block.y * block.z, 0 * sizeof(float)));
        CHECK_ERR;
        if (blocksPerSM > 0) {
  ...
  static bool shmemsizeset = false;
      if (!shmemsizeset) {
        CHECK_RES(hipFuncSetAttribute(reinterpret_cast<const void*>(&kernel_kernel_9656a8bcfa), hipFuncAttributeMaxDynamicSharedMemorySize, 0 * sizeof(float)));
        CHECK_ERR;
        shmemsizeset = true;
      }
      
  hipStream_t stream = (streamPtr != nullptr) ? static_cast<hipStream_t>(streamPtr) : 0;
  hipLaunchKernelGGL(kernel_kernel_9656a8bcfa, grid, block, 0 * sizeof(float), stream,  m0,  m0_extraOffset,  m1,  m1_extraOffset,  numElements0,  flags0 );
  CHECK_ERR;
}
...
__launch_bounds__(256)
 kernel_kernel_9656a8bcfa(const float* m0, unsigned m0_extraOffset, float* m1, unsigned m1_extraOffset, size_t numElements0, unsigned* flags0 ) {
        ...
        if (allowed) {
          ...
          if (v4_lead < 16) {
            ...
            for (int32_t v6_k1 = 0; v6_k1 < 16; ++v6_k1) {
              ...
              glb_m1[(v4_lead + v12_a)] = (powf(v21_data, 3.0f));
            }
          }
        }
      }
