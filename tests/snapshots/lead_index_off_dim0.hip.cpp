...
#endif
inline constexpr tensorforge::LaunchInfo launch_info_kernel_79755e9815b14736 = {{32, 8, 1}, 32, 20, 1, 8, 0, false, true, 1};
tensorforge::LaunchConfig launch_config_kernel_79755e9815b14736(size_t numElements0, void* streamPtr = nullptr);
void launcher_kernel_79755e9815b14736(float * m0, size_t m0_extraOffset, const float * m1, size_t m1_extraOffset, const float * m2, size_t m2_extraOffset, size_t numElements0, unsigned * flags0 = nullptr, void* streamPtr = nullptr);
...
#endif
tensorforge::LaunchConfig launch_config_kernel_79755e9815b14736(size_t numElements0, void* streamPtr) {
  (void)numElements0;
  (void)streamPtr;
  dim3 block (32, 8, 1);
  static std::size_t gridsize = 0;
      if (gridsize == 0) {
        ...
        CHECK_RES(hipOccupancyMaxActiveBlocksPerMultiprocessor(&blocksPerSM, kernel_kernel_79755e9815b14736, block.x * block.y * block.z, 0 * sizeof(float)));
        CHECK_ERR;
        int gfxMajor = 0;
        ...
        if (gfxMajor >= 10 && (0 * sizeof(float)) > 0) {
          ...
          CHECK_RES(hipOccupancyMaxActiveBlocksPerMultiprocessor(&blocksNoLds, kernel_kernel_79755e9815b14736, block.x * block.y * block.z, 0));
...
void launcher_kernel_79755e9815b14736(float * m0, size_t m0_extraOffset, const float * m1, size_t m1_extraOffset, const float * m2, size_t m2_extraOffset, size_t numElements0, unsigned * flags0, void* streamPtr) {
  const tensorforge::LaunchConfig config = launch_config_kernel_79755e9815b14736(numElements0, streamPtr);
  dim3 block (config.block[0], config.block[1], config.block[2]);
  dim3 grid (config.grid[0], config.grid[1], config.grid[2]);
  static bool shmemsizeset = false;
      if (!shmemsizeset) {
        CHECK_RES(hipFuncSetAttribute(reinterpret_cast<const void*>(&kernel_kernel_79755e9815b14736), hipFuncAttributeMaxDynamicSharedMemorySize, config.sharedMemBytes));
        CHECK_ERR;
        shmemsizeset = true;
      }
  ...
  hipLaunchKernelGGL(kernel_kernel_79755e9815b14736, grid, block, config.sharedMemBytes, stream, m0Arg, m0_extraOffset, m1Arg, m1_extraOffset, m2Arg, m2_extraOffset, numElements0, flags0Arg);
  CHECK_ERR;
}
...
__launch_bounds__(256)
 kernel_kernel_79755e9815b14736(tensorforge::SpacePtr<float, tensorforge::GlobalMemspace> m0, size_t m0_extraOffset, tensorforge::SpacePtr<const float, tensorforge::GlobalMemspace> m1, size_t m1_extraOffset, tensorforge::SpacePtr<const float, tensorforge::GlobalMemspace> m2, size_t m2_extraOffset, size_t numElements0, tensorforge::SpacePtr<unsigned, tensorforge::GlobalMemspace> flags0) {
    ...
    // tensorforge-meta: {"fp":"float","launch":{"active_threads":20,"block":[32,8,1],"cooperative":false,"lead_width":1,"mults_per_block":8,"persistent":true,"sections":[{"barrier":false,"mults_per_block":8,"shared_elements":0}],"shared_bytes":0,"shared_elements":0,"threads_per_mult":32},"loops":[],"operands":[{"addressing":"strided","alias":"C","bbox":[[0,0],[20,9]],"name":"m0","ordered":false,"parts":1,"shape":[20,9],"variant":false},{"addressing":"strided","alias":"A","bbox":[[0,0],[1,20]],"name":"m1","ordered":false,"parts":1,"shape":[1,20],"variant":false},{"addressing":"strided","alias":"B","bbox":[[0,0],[1,9]],"name":"m2","ordered":false,"parts":1,"shape":[1,9],"variant":false}],"operations":[{"add":false,"dest":{"addressing":"strided","bbox":[[0,0],[20,9]],"is_tmp":false,"name":"m0","offset":[0,0],"shape":[20,9]},"kind":"multilinear","ops":[{"addressing":"strided","bbox":[[0,0],[1,20]],"is_tmp":false,"name":"m1","offset":[0,0],"shape":[1,20]},{"addressing":"strided","bbox":[[0,0],[1,9]],"is_tmp":false,"name":"m2","offset":[0,0],"shape":[1,9]}],"permute":[[0,1],[0,1]],"target":[[-1,0],[-1,1]]}],"version":"0.0.1\n"}
    {
      ...
      for (size_t v1_batchId0 = (threadIdx.y + blockDim.y * (blockIdx.x)); v1_batchId0 < numElements0; v1_batchId0 += (gridDim.x * blockDim.y)) {
        ...
        if (allowed) {
          ...
          float v36_data = r1[0];
          float v39_data = r2[0];
          r2[0] = (v39_data + (v35_data * (tensorforge::broadcast<32, 1, 0>(v36_data))));
          float v42_data = r1[1];
          float v45_data = r2[1];
          r2[1] = (v45_data + (v35_data * (tensorforge::broadcast<32, 1, 0>(v42_data))));
          float v48_data = r1[2];
          float v51_data = r2[2];
          r2[2] = (v51_data + (v35_data * (tensorforge::broadcast<32, 1, 0>(v48_data))));
          float v54_data = r1[3];
          float v57_data = r2[3];
          r2[3] = (v57_data + (v35_data * (tensorforge::broadcast<32, 1, 0>(v54_data))));
          float v60_data = r1[4];
          float v63_data = r2[4];
          r2[4] = (v63_data + (v35_data * (tensorforge::broadcast<32, 1, 0>(v60_data))));
          float v66_data = r1[5];
          float v69_data = r2[5];
          r2[5] = (v69_data + (v35_data * (tensorforge::broadcast<32, 1, 0>(v66_data))));
          float v72_data = r1[6];
          float v75_data = r2[6];
          r2[6] = (v75_data + (v35_data * (tensorforge::broadcast<32, 1, 0>(v72_data))));
          float v78_data = r1[7];
          float v81_data = r2[7];
          r2[7] = (v81_data + (v35_data * (tensorforge::broadcast<32, 1, 0>(v78_data))));
          float v84_data = r1[8];
          float v87_data = r2[8];
          r2[8] = (v87_data + (v35_data * (tensorforge::broadcast<32, 1, 0>(v84_data))));
          // glb_m0 = store{r>g}(r2);
          if (v26_lead < 20) {
            #pragma unroll
            for (int32_t v90_i1 = 0; v90_i1 < 9; ++v90_i1) {
              float v92_data = r2[v90_i1];
              glb_m0[(v26_lead + (v90_i1 * 20))] = v92_data;
            }
          }
        }
      }
