...
void kernel::matmulAB::execute() {
  ...
  launcher_kernel_386f7503c1(C, extraOffset_C, const_cast<const float **>(A),
                             extraOffset_A, const_cast<const float **>(B),
                             extraOffset_B, numElements, flags, streamPtr);
...
void kernel::matmulATB::execute() {
  ...
  assert(streamPtr !=
         reinterpret_cast<void *>(std::numeric_limits<uintptr_t>::max()));
  launcher_kernel_9dfefadc54(const_cast<const float **>(A), extraOffset_A,
                             const_cast<const float **>(B), extraOffset_B, C,
                             extraOffset_C, numElements, flags, streamPtr);
  streamPtr = reinterpret_cast<void *>(std::numeric_limits<uintptr_t>::max());
  flags = nullptr;
}
...
void kernel::matmulABT::execute() {
  ...
  assert(streamPtr !=
         reinterpret_cast<void *>(std::numeric_limits<uintptr_t>::max()));
  launcher_kernel_44a6a7e323(const_cast<const float **>(A), extraOffset_A, C,
                             extraOffset_C, const_cast<const float **>(B),
                             extraOffset_B, numElements, flags, streamPtr);
  streamPtr = reinterpret_cast<void *>(std::numeric_limits<uintptr_t>::max());
  flags = nullptr;
}
...
void kernel::matmulATBT::execute() {
  ...
  float *_buffer0;
  _tmp0 = _buffer0;
  launcher_kernel_86dd44de64(const_cast<const float **>(A), extraOffset_A, C,
                             extraOffset_C, const_cast<const float **>(B),
                             extraOffset_B, numElements, flags, streamPtr);
  streamPtr = reinterpret_cast<void *>(std::numeric_limits<uintptr_t>::max());
  flags = nullptr;
