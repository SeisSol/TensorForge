...
int estimateNumElements(int SizeA, int SizeB, int SizeC,
...
int main(int Argc, char *Arcv[]) {
  ...
  int NumElements = estimateNumElements(SizeA, SizeB, SizeC,
  ...
  std::cout << "Allocated Device Mem. GB: "
            << Driver.getDeviceAllocatedMemSize() << std::endl;
  Driver.SetUp();
  ...
  std::tie(ShuffledDeviceA, ShuffledDeviceB, ShuffledDeviceC) =
  ...
  tensorforge::reference::gemm(TransA, TransB, M, N, K, Alpha, &HostA[OffsetA],
                               Lda, &HostB[OffsetB], Ldb, Beta, &HostC[OffsetC],
                               Ldc, NextA, NextB, NextC, NumElements);
  ...
  std::cout << "Computed Flops: " << NumRepeats * FlopCounter * NumElements
            << std::endl;
