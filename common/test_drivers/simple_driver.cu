...
void TestDriver::SetUp() {
  if (m_IsReady && !m_IsSet) {
    ...
    cudaMalloc(&m_DeviceMatA, m_SizeMatA * m_NumElements * sizeof(real)); CHECK_ERR;
    cudaMalloc(&m_DeviceMatB, m_SizeMatB * m_NumElements * sizeof(real)); CHECK_ERR;
    cudaMalloc(&m_DeviceMatC, m_SizeMatC * m_NumElements * sizeof(real)); CHECK_ERR;
    m_IsSet = true;
...
void TestDriver::prepareData() {
  if (m_IsSet) {
    ...
    cudaMemcpy(m_DeviceMatA, m_HostMatA, m_SizeMatA * m_NumElements * sizeof(real), cudaMemcpyHostToDevice);
    CHECK_ERR;
    cudaMemcpy(m_DeviceMatB, m_HostMatB, m_SizeMatB * m_NumElements * sizeof(real), cudaMemcpyHostToDevice);
    CHECK_ERR;
    cudaMemcpy(m_DeviceMatC, m_HostMatC, m_SizeMatC * m_NumElements * sizeof(real), cudaMemcpyHostToDevice);
    CHECK_ERR;
  }
  else {
...
std::tuple<real*, real*> TestDriver::getRawResults() {
  if (m_IsSet) {
    cudaMemcpy(m_ResultsFromDevice, m_DeviceMatC, m_SizeMatC * m_NumElements * sizeof(real), cudaMemcpyDeviceToHost);
    CHECK_ERR;
...
void TestDriver::TearDown() {
  if (m_IsSet) {
    ...
    cudaFree(m_DeviceMatA);
    cudaFree(m_DeviceMatB);
    cudaFree(m_DeviceMatC);
    m_IsSet = false;
  }
  else {
    throw std::string("Test hasn't been set. Call SetUp method first");
