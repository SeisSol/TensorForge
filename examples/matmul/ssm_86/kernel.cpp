...
namespace yateto {
  ...
  void kernel::matmulAB::execute() {
    ...
    launcher_kernel_15b8c0b6fd(const_cast<const float**>(B), extraOffset_B, C, extraOffset_C, const_cast<const float**>(A), extraOffset_A, numElements, flags, streamPtr);
  ...
  void kernel::matmulATB::execute() {
    ...
    assert(numElements != 0);
    assert(streamPtr != reinterpret_cast<void*>(std::numeric_limits<uintptr_t>::max()));
    launcher_kernel_b0502b5c17(const_cast<const float**>(B), extraOffset_B, const_cast<const float**>(A), extraOffset_A, C, extraOffset_C, numElements, flags, streamPtr);
    streamPtr = reinterpret_cast<void*>(std::numeric_limits<uintptr_t>::max());
    flags = nullptr;
  }
  ...
  void kernel::matmulABT::execute() {
    ...
    assert(numElements != 0);
    assert(streamPtr != reinterpret_cast<void*>(std::numeric_limits<uintptr_t>::max()));
    launcher_kernel_06c3e4ee5d(C, extraOffset_C, const_cast<const float**>(B), extraOffset_B, const_cast<const float**>(A), extraOffset_A, numElements, flags, streamPtr);
    streamPtr = reinterpret_cast<void*>(std::numeric_limits<uintptr_t>::max());
    flags = nullptr;
  }
  ...
  void kernel::matmulATBT::execute() {
    ...
    float* _buffer0;
    _tmp0 = _buffer0;
    launcher_kernel_0ac7dd0fbd(C, extraOffset_C, const_cast<const float**>(B), extraOffset_B, const_cast<const float**>(A), extraOffset_A, numElements, flags, streamPtr);
    streamPtr = reinterpret_cast<void*>(std::numeric_limits<uintptr_t>::max());
    flags = nullptr;
  }
} // namespace yateto
